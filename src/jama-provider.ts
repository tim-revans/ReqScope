...
async function getItemID(
  ...
  if (!abstractResponse.ok) {
    ...
    console.error(
      `Jama AbstractItem API Error (${abstractResponse.status}):`,
      errorBody,
    );
    return null;
  }
  ...
  const abstractData =
    (await abstractResponse.json()) as JamaAbstractSearchResponse;
...
async function getItem(
  ...
  apiRoot: string,
  accessToken: string,
): Promise<JamaSingleItemResponse | null> {
  ...
  if (!itemResponse.ok) {
    ...
    console.error(
      `Jama Item Detail API Error (${itemResponse.status}):`,
      errorBody,
    );
    return null;
  }
...
export class jamaProvider implements RequirementProvider {
  ...
  async fetchRequirement(id: string): Promise<RequirementData | null> {
    try {
      ...
      const credentials = Buffer.from(`${clientID}:${clientAuth}`).toString(
        "base64",
      );
      ...
      const [statusName, assigneeName] = await Promise.all([
        statusId
          ? this.fetchStatusName(statusId, apiRoot, accessToken)
          : Promise.resolve(undefined),
        assigneeId
          ? this.fetchUserName(assigneeId, apiRoot, accessToken)
          : Promise.resolve(undefined),
      ]);
